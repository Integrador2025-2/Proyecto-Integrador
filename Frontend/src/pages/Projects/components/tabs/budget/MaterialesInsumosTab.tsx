...
import { Package, Layers, Clock, AlertCircle } from 'lucide-react'
...
export default function MaterialesInsumosTab({ projectId, rubros }: MaterialesInsumosTabProps) {
    ...
    useEffect(() => {
        ...
        const loadMateriales = async () => {
            try {
                ...
                const data = await apiService.getMaterialesInsumosByRubro(materialesRubro.rubroId)
                setMaterialesData(data)
            } catch (err) {
                const message =
                    err instanceof Error
                        ? err.message
                        : 'Error al cargar materiales e insumos del proyecto'
                setError(message)
                console.error('Error loading materiales e insumos:', err)
            } finally {
                setIsLoading(false)
            }
        }
        ...
        void loadMateriales()
    }, [materialesRubro?.rubroId, projectId])
    ...
    const getTotalPresupuesto = () =>
        materialesData.reduce((sum, item) => sum + (item.total || 0), 0)

    if (!materialesRubro) {
        return (
            <div className="min-h-[200px] flex items-center justify-center">
                <p className="text-sm text-gray-600">
                    No se encontró el rubro &quot;Materiales e Insumos&quot; en la configuración de
                    rubros.
                </p>
            </div>
        )
        ...
        return (
            <div className="min-h-[200px] flex items-center justify-center">
                <p className="text-gray-600 text-sm">
                    Cargando información de materiales e insumos...
                </p>
            </div>
        )
    }
        ...
        return (
            <div className="min-h-[200px] flex flex-col items-center justify-center gap-2 text-center">
                <p className="text-red-600 text-sm font-medium">
                    Error al cargar materiales e insumos
                </p>
        ...
        return (
            <div className="min-h-[200px] flex flex-col items-center justify-center gap-2 text-center border border-dashed border-gray-300 rounded-2xl bg-gray-50 px-4">
                ...
                <p className="text-sm font-semibold text-gray-800">
                    Este proyecto aún no tiene materiales o insumos registrados.
                </p>
                <p className="text-xs text-gray-600 max-w-md">
                    Cuando se registren materiales e insumos desde el backend, aparecerán aquí con
                    su descripción y presupuesto.
                </p>
            </div>
        )
    }
    ...
    return (
        <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-lg font-bold text-gray-900">
                            {formatCurrency(getTotalPresupuesto() / materialesData.length)}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4">
                ...
                {materialesData.some((m) => m.ragEstado && m.ragEstado.toLowerCase() !== 'ok') && (
                    <div className="mt-3 flex items-start gap-2 text-xs text-yellow-700 bg-yellow-50 border border-yellow-200 rounded-lg px-3 py-2">
                        ...
                        <p>
                            Existen materiales o insumos marcados con un estado distinto a
